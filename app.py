...
# [안전 패치 1] 어떤 에러가 나든 화면 레이아웃과 버튼은 무조건 먼저 그려지도록 순서를 바꿨습니다.
...
# [안전 패치 2] 스트림릿 금고(Secrets)에 키가 잘 들어있는지 안전하게 먼저 확인하는 장치입니다.
has_key = True
if "GROQ_API_KEY" not in st.secrets:
    st.warning("⚠️ 스트림릿 금고(Secrets)에 'GROQ_API_KEY' 이름표를 가진 열쇠가 아직 없는 것 같습니다. 금고 설정을 확인해 주세요!")
    has_key = False

# 이 버튼은 이제 무조건 화면에 뜹니다!
if st.button("🚀 오늘 아침 시황 리포트 생성하기", type="primary"):
    if not has_key:
        st.error("❌ 금고에 GROQ_API_KEY가 등록되지 않아 AI를 깨울 수 없습니다. 설정을 먼저 완료해 주세요.")
    else:
        with st.spinner("AI가 실시간 글로벌 증시와 반도체 뉴스를 분석 중입니다..."):
            try:
                ...
                if result_text:
                    ...
                    st.error("😭 현재 Groq 무료 서버가 일시적인 과부하 상태입니다. 잠시 후 다시 시도해 주세요.")
